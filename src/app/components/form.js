...
export const Form = ({ initialRef }) => {
    ...
    const [mounted, setMounted] = useState(false); // New state to check if mounted

    const router = useRouter(); // Initialize router

    useEffect(() => {
        setMounted(true); // Ensures router is available on client side after component mounts
    }, []);
    ...
    const handleContactFormSubmit = async (e) => {
        ...
        if (!mounted) return; // Don't proceed if the component is not mounted yet

        // Check the router pathname to set campaign state
        ...
        const endpoint = "https://ke37371vfe.execute-api.us-east-1.amazonaws.com/default/sendContactEmailFryTech";
        ...
        <>
            <form onSubmit={handleContactFormSubmit}>
                <div className="flex flex-col relative">
                    ...
                    <div>
                        <label className="uppercase tracking-wide text-xs">Comments?</label>
                        <textarea
